...
export default class Transaction extends Model {
  async credit(account, amount, cashierId, receipient) {
    ...
    try {
      const { rows } = await this.insert('accountNumber, cashier, transactionType, amount, oldbalance, newbalance,donor, receipient', '$1, $2, $3, $4, $5, $6, $7, $8', [
  ...
  async getTransactionById(id) {
    try {
      const { rows } = await this.selectWithJoin(
        'trans.id, trans.createdOn, transactiontype, trans.accountNumber, amount, oldBalance, newBalance, owner,donor,receipient',
        'trans.id=$1',
        [id],
      );
      return rows[0];
